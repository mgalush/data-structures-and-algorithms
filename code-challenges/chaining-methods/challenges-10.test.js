...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => res.send(`I'm a greeting!`));
  app.get('/aboutme', (req, res) => res.send(`My name is Melissa`));
  app.get('/favoritefoods', (req, res) => res.send(['bananas', 'pancakes', 'pizza']));
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((arr) => {
    ...
    }).map((returnVal) => Math.pow(2, returnVal));
  })
};
...
let findShortest = (data) => {
  const shortestCharacter = data.reduce((shortestCharacterSoFar, currentCharacter) => {
    if (!shortestCharacterSoFar) {
      return currentCharacter;
    }
    return (parseInt(shortestCharacterSoFar.height) < parseInt(currentCharacter.height))
      ? shortestCharacterSoFar 
      : currentCharacter;
  });
